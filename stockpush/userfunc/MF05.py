...
def MF05(symbol=None, period=None, start=None, end=None, param_set_id=None, **_kw):
    """自定义函数系统适配入口。兼容三种调用：

    1) 引擎调用: MF05(symbol="601336", period="5m", start="...", end="...", param_set_id=0)
    2) 测试 (4.4): MF05(df) — df 被忽略，自动取第一只自选股计算
    3) 正式: MF05("601336") — 指定 symbol 计算
    ...
    try:
        result = mf05_calculate(symbol, period or "5m", start or "", end or "", param_set_id or 0)
