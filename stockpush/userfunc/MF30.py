...
def _compute(df, df_daily, *,
    ...
    参数含义同通达信 MF30V2 INPUT (N1..N4, m1..m4, MP1..MP4, LONGP, SHORTP, M, LH, LL, PP)。
    """
    # 1. 当前周期 (30M)
    P = _P(df)
...
def _fetch_data(symbol, min_30m=None, min_daily=None):
    ...
    sql = f"SELECT ts, open, high, low, close FROM tb_raw_30m WHERE symbol = '{symbol}' ORDER BY ts"
    ...
    sql_d = f"SELECT ts, open, high, low, close FROM tb_raw_1d WHERE symbol = '{symbol}' ORDER BY ts"
...
def mf30(symbol, *,
    ...
    关键字参数 — 全部通达信 MF30V2 INPUT，默认值按原公式:
