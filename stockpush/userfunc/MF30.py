"""
MF30V2 多因子三级买卖信号 + 第四类RSI买卖点
源码对照: F51/docs/MF30v2.txt

用法:
  sig = mf30("601336")                           # symbol → 自动从 PostgreSQL 取数
  sig = mf30("601336", N1=40, SHORTP=30)         # 自定义参数
  sig["mabuy"], sig["masell"]

  # 或用预取 DataFrame
  from stockpush.userfunc import mf30_from_df
  sig = mf30_from_df(df_30m, df_daily)
"""
...
def _realign(s, target_index):
    """将日线 Series 向前填充到主力周期索引 (跨周期 #day 语义)"""
...
def _compute(df, df_daily, *,
            ...
            LH=72, LL=30, PP=13):
    """
    MF30 核心计算。接收 30M + 日线 DataFrame，返回信号 dict。
    参数含义同 MF30V2 INPUT (N1..N4, m1..m4, MP1..MP4, LONGP, SHORTP, M, LH, LL, PP)。
    """
    # 1. 当前周期 (30M)
    P = _P(df)
...
def mf30_min_bars():
    """返回 mf30 正确计算需要的最少数据量（行数），默认参数下。
    30M: MACD(EMA(75)需74行 + EMA(DIFF,9)需8行) = 83
          hrama(N1=35,m1=9) w2=WMA(9)=8行
          ugpdx(N=8,m1=10,m2=28) HHV(28)+EMA(8)=34行
          → max(83,9,34) = 83
    日线: hrama(N4=45,m4=27) w2=WMA(27)=26行 + HHV(X,5)=4行 = 30
          → 30
    安全系数 3x: 30M = 83*3=249, 日线 = 30*3=90"""
    return {'main': 83 * 3, 'daily': 30 * 3}


# ── 从 PostgreSQL 取数 ──────────────────────────────────────

def _fetch_data(symbol, min_30m=None, min_daily=None):
    """从 PostgreSQL 获取 symbol 的 30M + 日线数据。

    Args:
        symbol: 股票代码
        min_30m: 限制取尾部 N 行 30M 数据（None=全量）
        min_daily: 限制取尾部 N 行日线数据（None=全量）
    """
    from stockpush.pg_connector import PGConnector
    conn = PGConnector()

    sql = (f"SELECT ts, open, high, low, close FROM tb_raw_30m "
           f"WHERE symbol = ? ORDER BY ts")
    params_30m = (symbol,)
    if min_30m is not None:
        sql = f"SELECT * FROM ({sql} DESC LIMIT {min_30m}) sub ORDER BY ts"
    rows = conn.execute_query(sql, params_30m)
    ...
    sql_d = (f"SELECT ts, open, high, low, close FROM tb_raw_1d "
             f"WHERE symbol = ? ORDER BY ts")
    params_1d = (symbol,)
    if min_daily is not None:
        sql_d = f"SELECT * FROM ({sql_d} DESC LIMIT {min_daily}) sub ORDER BY ts"
    rows_d = conn.execute_query(sql_d, params_1d)
    dfd = pd.DataFrame(rows_d)
    if dfd.empty:
        raise ValueError(f"tb_raw_1d 无 {symbol} 数据")
    for c in ['open', 'high', 'low', 'close']:
...
def mf30(symbol, *,
        ...
        LH=72, LL=30, PP=13,
        min_30m=None, min_daily=None):
    """
    从 PostgreSQL 取 symbol 数据后计算 MF30V2 信号。

    参数:
        symbol — 股票代码，如 "601336"
        min_30m / min_daily — 限制取尾部行数（None=全量）
                              mf30_min_bars() 返回值可用作下限

    关键字参数 — 全部 MF30V2 INPUT，默认值按原公式:
        N1..N4, m1..m4, MP1..MP4 — hrama 参数
        LONGP / SHORTP / M       — MACD 参数
        LH / LL / PP             — 第四类买卖点 RSI 参数

    返回 dict (见 mf30_from_df)
    """
    df, df_daily = _fetch_data(symbol, min_30m=min_30m, min_daily=min_daily)
    return mf30_from_df(df, df_daily, N1=N1, N2=N2, N3=N3, N4=N4,
                        m1=m1, m2=m2, m3=m3, m4=m4,
                        MP1=MP1, MP2=MP2, MP3=MP3, MP4=MP4,
                        LONGP=LONGP, SHORTP=SHORTP, M=M,
                        LH=LH, LL=LL, PP=PP)

def mf30_from_df(df, df_daily, *,
                N1=35, N2=35, N3=35, N4=45,
                m1=9, m2=15, m3=9, m4=27,
                MP1=2, MP2=2, MP3=2, MP4=2,
                LONGP=75, SHORTP=25, M=9,
                LH=72, LL=30, PP=13):
    """
    接收预取的 30M / 日线 DataFrame 计算 MF30V2 信号。

    参数:
        df       — 30M K 线 (须含 open/high/low/close, DateTimeIndex)
        df_daily — 日线 K 线

    关键字参数同上。

    返回 dict:
        xx1..xx4      — 4 条均线 (xx1/xx2=30M, xx3/xx4=日线)
        macd2         — MACD 柱
        R1, R2, RSI1  — 摆动指标
...
def mf30_calculate(symbol: str, period: str, start: str, end: str,
                   param_set_id: int = 0) -> dict:
    """MF30V2 信号计算（新契约）。

    自动从 PostgreSQL 取 30M+日线数据，计算后返回标准信号格式。
    ...
    params = registry.get_params('mf30', param_set_id)
    ...
    # 调用原有计算
    result = mf30_from_df(df, dfd, **typed)
