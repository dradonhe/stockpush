...
用法:
  sig = mm2_i5_min1("601336")
  sig = mm2_i5_min1("601336", TH=23)
  sig["buy"], sig["sell"]

  # 或用预取 DataFrame
  from stockpush.userfunc import mm2_i5_min1_from_df
  sig = mm2_i5_min1_from_df(df_daily)
"""
...
def _compute(df, *,
    ...
    _B1A = (CORE_B | RB3) & (T_SEP1 > 0) & (T_SEP1 <= 15) & B1A_FLT & (RSIH < 50) & CH_FILT_BUY
    ...
    _S2 = CORE_S & (_HHV(H, 10) == SG2) & CH_B2_BEAR & (T_SEP2S > 0) & (RSIH > 45) & CH_FILT_SELL
...
def mm2_i5_min1_min_bars():
    """返回 mm2_i5_min1 正确计算需要的最少数据量（行数），默认参数下。
...
def mm2_i5_min1(symbol, *, MM1=30, MM2=150, MM3=750, MM4=3750, TH=23, min_daily=None):
    """一站式入口：取日线 + 计算。
...
def mm2_i5_min1_from_df(df, *, MM1=30, MM2=150, MM3=750, MM4=3750, TH=23):
    """从预取 DataFrame 计算"""
    return _compute(df, MM1=MM1, MM2=MM2, MM3=MM3, MM4=MM4, TH=TH)
...
def mm2_i5_min1_calculate(symbol: str, period: str, start: str, end: str,
    ...
    """MM2 I5 min1 信号计算（新契约，含mm3/mm4方向过滤）。
    ...
    params = registry.get_params('mm2_i5_min1', param_set_id)
...
def MM2_I5_MIN1(symbol=None, *, period="1d", start="", end="", param_set_id=0, **_kw):
    """自定义函数系统适配入口。

    正式调用: MM2_I5_MIN1(symbol="601336", period="1d", ...)
    测试场景: MM2_I5_MIN1() — 自动取第一只自选股
    ...
    try:
        result = mm2_i5_min1_calculate(symbol, period, start, end, param_set_id)
        signals = result.get("signals", [])
    except Exception as e:
        log.warning("MM2_I5_MIN1(%s) 计算失败: %s", symbol, e)
        signals = []
